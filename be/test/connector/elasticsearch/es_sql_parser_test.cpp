...
TEST_F(EsSqlParserTest, DatetimeFractionalSecondsAreMilliseconds) {
    ...
    auto sub_usec = [&](size_t r) { return col->get(r).get_timestamp().to_unix_microsecond() % 1000000; };
    ...
    EXPECT_EQ(expected.to_unix_microsecond(), col->get(0).get_timestamp().to_unix_microsecond());
}
